...
fn prepare_normal_test_cases(path: &str) -> ServiceResult<i32> {
    ...
    loop {
        ...
        test_cases.insert(id.to_string(), NormalTestCaseInfo{
...
fn prepare_spj_test_case(path: &str) -> ServiceResult<i32> {
    ...
    loop {
        ...
        test_cases.insert(count.to_string(), SpjTestCaseInfo{
            input_name: name,
...
pub fn prepare_test_cases(path: &str, is_spj: bool) -> ServiceResult<i32> {
    if is_spj { Ok(prepare_spj_test_case(path)?) } 
    else { Ok(prepare_normal_test_cases(path)?) }
}