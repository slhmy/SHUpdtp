...
pub fn insert(conn: &PgConnection, insertable_user: &InsertableUser) -> ServiceResult<()> {
...
pub fn get_by_id(conn: &PgConnection, id: i32) -> ServiceResult<User> {
    Ok(users_schema::table
        .filter(users_schema::id.eq(id))
        .first::<User>(conn)?)
}

pub fn get_by_account(conn: &PgConnection, account: String) -> ServiceResult<User> {
    Ok(users_schema::table
        .filter(users_schema::account.eq(account))
        .first::<User>(conn)?)
}

pub fn update_by_id(conn: &PgConnection, id: i32, user_form: UserForm) -> ServiceResult<()> {
...
pub fn delete_by_id(conn: &PgConnection, id: i32) -> ServiceResult<()> {
